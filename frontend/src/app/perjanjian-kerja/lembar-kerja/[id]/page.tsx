...
export default function CreateWorksheet() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const worksheetData = worksheetResponse.data.worksheet;
        setFormData({
          ...worksheetData,
          attachments: worksheetData.attachments, // mapping manual
        });

        if (worksheetData.order_date) {
          setOrderDate(new Date(worksheetData.order_date));
        }
        if (worksheetData.deadline_date) {
          setDeadlineDate(new Date(worksheetData.deadline_date));
        }
        if (worksheetData.appearers) {
          setSelectedAppearers(
            worksheetData.appearers.map((a: any) => a.appearer_id)
          );
        }
        setAttachments(worksheetData.attachments || []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.post(`/worksheet-notaries/${id}?_method=PUT`, formDataToSend, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      showSuccess("Worksheet updated successfully!");
      router.push("/perjanjian-kerja/lembar-kerja");
    } catch (error: any) {
      if (error.response?.status === 422) {
        showValidationErrors(error.response.data.errors);
      } else {
        showError("Failed to update worksheet!");
      }
    } finally {
      setSaving(false);
    }
            ...
            {attachments.length > 0 && (
              <div className="pt-4 border-t border-slate-200">
                <Button
                  variant="outline"
                  onClick={addAttachment}
                  className="w-full cursor-pointer gap-2 border-dashed border-slate-300 hover:border-blue-400 hover:bg-blue-50 text-slate-600 hover:text-blue-600 transition-colors bg-transparent"
                >
                  <Plus className="w-4 h-4" />
