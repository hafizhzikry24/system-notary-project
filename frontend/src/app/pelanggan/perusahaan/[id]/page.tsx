...
import { Paperclip, Plus, X, Upload, FileText } from "lucide-react";
...
// ------------------- Component -------------------
export default function UpdateCustomerCompany() {
  ...
  const [attachments, setAttachments] = useState<
    (CustomerCompanyAttachment & { file?: File | null })[]
  >([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [custCompany] = await Promise.all([
          api.get(`/customer-companies/${id}`),
        ]);

        const customer = custCompany.data.customer_company;
        setFormData({
          ...customer,
  ...
  const handleInputChange = (name: keyof CustomerCompany, value: string) => {
    setFormData((prev) => ({
      ...prev,
      [name]: value,
    }));
  ...
  const updateAttachment = (
    index: number,
    field: keyof (CustomerCompanyAttachment & { file?: File | null }),
    value: any
  ) => {
    setAttachments((prev) =>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.post(`/customer-companies/${id}?_method=PUT`, formDataToSend, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      showSuccess("Customer perusahaan updated successfully!");
      router.push("/pelanggan/perusahaan");
    } catch (error: any) {
      if (error.response?.status === 422) {
        showValidationErrors(error.response.data.errors);
      } else {
        showError("Failed to updated customer Perusahaan!");
      }
    } finally {
      setSaving(false);
    }
      ...
      <Layout>
        <div className="container mx-auto px-16 py-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <LabelInputContainer>
                <Label htmlFor="license_number">No Identitas Perusahaan</Label>
                <Input
                  id="license_number"
                  type="number"
                  ...
                  placeholder="No Identitas Perusahaan"
                />
              </LabelInputContainer>
              <LabelInputContainer>
                <Label htmlFor="deed_number">No Akta Pendirian</Label>
                <Input
                  id="deed_number"
                  type="number"
                  value={formData.deed_number || ""}
                  onChange={(e) =>
                    handleInputChange("deed_number", e.target.value)
                  }
                  placeholder="No Akta Pendirian"
                />
              </LabelInputContainer>
            </div>
            <LabelInputContainer>
            ...
            <div>
              <Dialog open={open} onOpenChange={setOpen}>
                <DialogTrigger asChild>
                  <Button
                    variant="outline"
                    className="gap-2 hover:bg-slate-50 cursor-pointer transition-colors bg-transparent"
                  >
                          ...
                          >
                            <div className="flex items-start gap-4">
                              ...
                              <Button
                                ...
                                className="cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity text-slate-400 hover:text-red-500 hover:bg-red-50"
                              >
                                <X className="w-4 h-4" />
                              </Button>
                            </div>
                  ...
                  {attachments.length > 0 && (
                    <div className="pt-4 border-t border-slate-200">
                      <Button
                        ...
                        className="cursor-pointer w-full gap-2 border-dashed border-slate-300 hover:border-blue-400 hover:bg-blue-50 text-slate-600 hover:text-blue-600 transition-colors bg-transparent"
                      >
                        <Plus className="w-4 h-4" />
            ...
            <Button type="submit" className="w-full cursor-pointer" disabled={saving}>
              {saving ? "Updating..." : "Updated Customer Perusahaan"}
            </Button>
          </form>
        </div>
      </Layout>
